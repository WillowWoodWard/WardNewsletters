/* Generated by Ward Newsletter Generator. */
import './App.css'

const newsletters = [{"periodKey":"2026-09","monthLabel":"September 2026","title":"Willow Wood Ward - September 2026","url":"/n/sfVmImYb6j_f9zRGHWOy0Imo/"},{"periodKey":"2026-10","monthLabel":"October 2026","isComingSoon":true}]

function App() {
  return (
    <div className="archive-shell">
      <header className="archive-header">
        <div className="archive-brand">Willow Wood Ward</div>
        <a className="archive-qr-link" href="/WardNewsletters/">Newsletter home</a>
      </header>
      <main className="archive-main">
        <p className="archive-kicker">The ward newsletter archive</p>
        <h1 className="archive-title">A place to stay connected.</h1>
        <p className="archive-intro">Every monthly newsletter, gathered in one reliable place. This address will stay the same as new issues are added.</p>
        {newsletters.length > 0 ? (
          <section className="archive-list" aria-label="Monthly newsletters">
            {newsletters.map((newsletter) => newsletter.isComingSoon ? (
              <div className="archive-card" key={newsletter.periodKey}>
                <div>
                  <div className="archive-card-month">{newsletter.monthLabel}</div>
                  <h2 className="archive-card-title">Coming soon</h2>
                </div>
                <div className="archive-card-action">New issue in progress</div>
              </div>
            ) : (
              <a className="archive-card" href={newsletter.url} key={newsletter.periodKey} target="_blank" rel="noopener noreferrer">
                <div>
                  <div className="archive-card-month">{newsletter.monthLabel}</div>
                  <h2 className="archive-card-title">{newsletter.monthLabel.replace(/\s+\d{4}$/, '')}</h2>
                </div>
                <div className="archive-card-action">Read newsletter &#8594;</div>
              </a>
            ))}
          </section>
        ) : <p className="archive-empty">The first newsletter will appear here after it is published.</p>}
      </main>
    </div>
  )
}

export default App
